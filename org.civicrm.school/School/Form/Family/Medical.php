...
class School_Form_Family_Medical extends School_Form_Family {
	...
	$relationships = civicrm_api( 'relationship','get',$relParams );
        ...
        if ( !$relationships['is_error'] && is_array($relationships['values']) ) {
            foreach ( $relationships['values'] as $rid => $rFields ) {
                $this->_relId = $rid;
                break;
            }
        } else {
    ...
    function setDefaultValues( ) 
    {
        ...
        $query = "SELECT * FROM ". School_Form_Family::MEDICAL_DETAILS_TABLE . " WHERE entity_id = %1"; 
	...
	$relationships = civicrm_api( 'relationship','get',$relParams);
            if ( ! $relationships['is_error'] ) {
                foreach ( $relationships['values'] as $relVal ) {
                    $siblingId = $relVal['contact_id_b'];
                    break;
                }
    ...
    function buildQuickForm( ) {
        $enabledSections = CRM_Core_BAO_Persistent::getContext( 'school family config', 'medical tab sections' );
        if( empty($enabledSections) || in_array( 'insurance', $enabledSections )){
            $infoGroupTree = CRM_Core_BAO_CustomGroup::getTree( 'Individual',
                                                                $this,
                                                                $this->_studentId,
                                                                $this->_medInfoId );
        ...
        if( empty($enabledSections) || in_array( 'counseling', $enabledSections )){          
            $this->add( 'checkbox', 'counselor_authorization', ts('Counselor Authorization'), null, false );
        }
        
        if( empty($enabledSections) || in_array( 'health', $enabledSections )){
            // for Allergies
        ...
        //check if authorization section is enabled
        if( !empty($enabledSections) && !in_array( 'counseling', $enabledSections )){
            unset( $buttons[0]['js'] );
        }
        ...
        $this->addFormRule( array( 'School_Form_Family_Medical', 'formRule' ) );
    }
    ...
    function postProcess() 
    {
        ...
        $customParams = array( );
        $count = 1;
        if ( !empty( $this->_infoElementMapper ) ) {
        ...
        $query = "DELETE FROM " . School_Form_Family::MEDICAL_DETAILS_TABLE . " WHERE entity_id = %1";
        $contactParams = array( 1 => array( $this->_studentId, 'Integer') );
