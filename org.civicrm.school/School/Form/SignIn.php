...
class School_Form_SignIn extends CRM_Core_Form {
	...
	if( empty($term) ) {
	  $term = null;
        ...
        $term = School_Utils_ExtendedCare::getTerm( $term ); 
...
";
	    ...
	    if( empty($dao->sout_class)){
	      $courseName = $dao->course_name;
	    }else{
	       $courseName = $dao->sout_class ;
	    }
            if ( empty( $courseName ) ) {
        ...
        $students =
            ...
            School_Utils_Query::getStudentsByGrade( true, false, true , ''  );
        ...
        $classes = array( '' => '- Select Class -' ) + School_Utils_Query::getClasses( );
