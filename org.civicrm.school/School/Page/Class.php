...
class School_Page_Class extends CRM_Core_Page {
    ...
    function run( ) {
        ...
        if ( $action  && ( array_key_exists( $action, self::actionLinks( ) ) || ( $action & CRM_Core_Action::ADD ) ) ) {
            ...
            $controller =& new CRM_Core_Controller_Simple( 'School_Form_Class' ,'Configure Class');
            $controller->process( );
    ...
    { 
        ...
        if ( $permission ) {
            $classInfo = School_Utils_ExtendedCare::getClassCount( null, true, $this->_term );
        }

        $activities =  array( );
        $activities =& School_Utils_ExtendedCare::getActivities( null,
                                                              CRM_Core_DAO::$_nullObject,
                                                              true,
                                                              $this->_term );
        $actionEnable =  CRM_Core_Action::UPDATE;
        $actionEnable  += ( CRM_Core_Action::DISABLE);
        $values  = array( );
        foreach ( $activities as $day => &$dayValues ) {
            ...
            foreach ( $dayValues as $session => &$sessionValues ) {
                foreach ( $sessionValues['details'] as $id => &$idValues ) {
                    ...
                    $idValues['session'] = School_Utils_ExtendedCare::getTime( $idValues['session'] );
                    $values[$day][] =& $idValues;
                }
            }
        }
        ...
        if( $permission ) {
            ...
            $disableActivities =& School_Utils_ExtendedCare::getActivities( null,
                                                                         CRM_Core_DAO::$_nullObject ,
                                                                         false );
            $actionDisable = CRM_Core_Action::UPDATE ;
            $actionDisable  += ( CRM_Core_Action::ENABLE);
            $disable = array( );
            foreach ( $disableActivities as $day => $valueDay ) {
                ...
                foreach ( $valueDay as $session => $valueSession ) {
                    foreach ( $valueSession['details'] as $id => $valueId ) {
                        ...
                        $valueId['session'] = School_Utils_ExtendedCare::getTime( $valueId['session'] );
                        $disable[$day][] = $valueId;
                    }
                }
            }
