...
class School_Page_Detail extends CRM_Core_Page {
...
";
        ...
        $params = array( 1 => array( $name, 'String' ),
                         2 => array( School_Utils_ExtendedCare::getTerm( ),
                                     'String' ),
                         3 => array( $day, 'String' ),
                         4 => array( $sess, 'String' ) );
        ...
        while ( $dao->fetch( ) ) {
            $values[] = array( 'contact_id'   => $dao->id,
                               ...
                               'url'          => CRM_Utils_System::url( 'civicrm/profile/view',
                                                                        "reset=1&gid={$gidStudent}&id={$dao->id}" ) );
        }

        $this->assign_by_ref( 'values', $values );

        $classDetails = array( 'name' => $name,
                               'time' => School_Utils_ExtendedCare::getTime( $sess ),
                               'day'  => $day );
