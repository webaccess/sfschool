...
class School_Utils_Conference {
  ...
  function getAdvisorRelTypeId( ) {
    $advisorRelTypeId = CRM_Core_DAO::getFieldValue( "CRM_Contact_DAO_RelationshipType", "Advisor", "id", "name_b_a" );
    return $advisorRelTypeId;
  }

  function getConferenceActTypeId( ) {
    $conferenceActTypeId = CRM_Core_DAO::getFieldValue( "CRM_Core_DAO_OptionValue", "Parent Teacher Conference", "value", "name" );
    return $conferenceActTypeId;
  }
...
";
  ...
  function sendConferenceEmail( $activityID, $advisorID, $childID, $dateTime = null ) {
    ...
    list( $templateVars['advisorName'],
      $templateVars['advisorEmail'] ) = School_Utils_Query::getNameAndEmail( $advisorID );
    ...
    School_Utils_Mail::sendMailToParents( $childID,
      'School/Mail/Conference/Subject.tpl',
      'School/Mail/Conference/Message.tpl',
      $templateVars );
  }
  ...
  function sendNotScheduledReminderEmail( $advisorID, $childID ) {
    ...
    require_once 'School/Utils/Mail.php';
    School_Utils_Mail::sendMailToParents( $childID,
      'School/Mail/Conference/NotScheduledSubject.tpl',
      'School/Mail/Conference/NotScheduledMessage.tpl',
      $templateVars );
  }
...
";
    ...
    while ( $dao->fetch( ) ) {
      ...
      if ($results['values'][0][$getbooking_startdate_id] <= $current_date && $results['values'][0][$getbooking_enddate_id]  >= $current_date ) {
        $values[$dao->contact_id_b]['meeting']['title'] = "Please schedule your {$dao->subject} with {$advisorName}. Online registraton will <strong>close Nov 6th</strong>";
  ...
  static function createConferenceSchedule( $params = NULL ) {
    ...
    if ( $params ) {
      self::createConference( $adminID, $teacherID,
        self::getConferenceActTypeId( ),
        date('YmdHis', strtotime("$date $start")),
        self::SUBJECT,
        self::LOCATION,
        self::STATUS,
        $duration );
...
";
    $params = array( 1 => array( self::getConferenceActTypeId( ), 'Integer' ),
...
";
    ...
    $params  = array( 1 => array( $teacherID, 'Integer' ),
                      2 => array( self::getAdvisorRelTypeId(), 'Integer' ) );
...
";
    $params  = array( 1 => array( $teacherID   , 'Integer' ),
               2 => array( self::getAdvisorRelTypeId( ), 'Integer' ),
               3 => array( self::getConferenceActTypeId( ) , 'Integer' ) );
...
";
  ...
  static function buildPTCForm( &$form, $staffID ) {
    ...
    $form->addDateTime(  "slot_date", ts( 'Date' ) );
  ...
  static function postProcessPTC( &$form, $advisorID ) {
    ...
    if ( ! empty( $params['slot_date'] ) ) {
      ...
      $date = CRM_Utils_Date::processDate( $params["slot_date"], $params["slot_date_time"] );
