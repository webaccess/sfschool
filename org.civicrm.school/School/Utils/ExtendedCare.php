...
class School_Utils_ExtendedCare {
  const
    TERM_POSITION = 0,
...
";
    ...
    while ( $dao->fetch( ) ) {
      ...
      if ( $dao->fee_block > 1 ) {
        $title .= " - {$dao->fee_block} activity blocks";
      }
  ...
  static function getTerm( $term = null ) {
    ...
    if ( $_term === null ) {
      $_term = defined( 'SCHOOL_TERM' ) ? SCHOOL_TERM : self::TERM;
    }
    return $_term;
  }
  ...
  static function makeID( &$dao, $class = 'Source' ) {
    ...
    return preg_replace('/\s+|\W+/', '_', trim( $id ));
  }
  ...
  static function sendExtendedCareEmail( $childID, $templateVars = array( ) ) {
    ...
    School_Utils_Mail::sendMailToParents( $childID,
      'School/Mail/ExtendedCare/Subject.tpl',
      'School/Mail/ExtendedCare/Message.tpl',
      $templateVars,
...
";
    ...
    while ( $dao->fetch( ) ) {
      ...
      if ( $dao->name ) {
        if ( ! empty($values[$dao->contact_id]['extendedCareDay'] )) {
    ...
    foreach ( $values as $contactID => $value ) {
      foreach ( $daysOfWeek as $day )  {
        if ( ! empty( $values[$contactID]['extendedCareDay'][$day]) && !empty( $values[$contactID]['extendedCare'])) {
      ...
      if ( is_numeric( $values[$contactID]['grade'] ) ) {
        ...
        $values[$contactID]['extendedCareEdit'] =
          CRM_Utils_System::url( 'civicrm/profile/edit', "reset=1&gid={$gidStudent}&id={$contactID}&excare=1&$parent" );
...
";
  ...
  static function &signoutDetails(
    $startDate        = null,
    $endDate          = null,
    $isMorning        = true,
...
";
    ...
    $freeClasses = array(
      'Volleyball', 'Cross Country', 'Amnesty International',
      'SMART', 'Yearbook',
      'Futsol Practice', 'Futsol', 'MS Futsal',
      'Basketball Team 3:30-5:00 p.m.', 'Band', 'Basketball/MS',
      'Counselor in Training',
      'Middle School Basketball', 'Middle School Athletics', 'Shakespeare',
      'Shakespeare Practice',
      'World Music', 'MS Basketball',
      'Junior Counselor', 'Junior Counselor/Preschool', 'Junior Counselor Plasticrafts',
      'Junior Counselor/Creative Kitchen', 'Junior Counselor/Fabric, Thread, and Yarn',
      'Junior Counselor/Faerie Houses', 'Junior Counselor/Textiles', 'Junior Counselor/Ceramics',
      ...
      'World Music Class',
      'Shakespeare/8th Grade Practice',
      'MS Athletics'
    );
...
";
    ...
    while ( $dao->fetch( ) ) {
      if ( $dao->entity_id != $currentEntityID &&
        ...
        School_Utils_Mail::sendMailToParents( $currentEntityID,
          'School/Mail/ExtendedCare/NotSignedOutSubject.tpl',
          'School/Mail/ExtendedCare/NotSignedOutMessage.tpl',
          $templateVars,
          self::COORDINATOR_EMAIL );
        $days = array( );
      }
  ...
  static function chargeMorningBlock( $time ) {
    ...
    require_once 'School/Utils/Date.php';
    $dateParts = School_Utils_Date::unformat( $time );
  ...
  static function sendBalanceInvoiceEmail( $cutoff = 10 ) {
    ...
    foreach ( $details as $id =>& $value ) {
      ...
      // now send a message to the parents about what they did
      School_Utils_Mail::sendMailToParents( $id,
        'School/Mail/ExtendedCare/InvoiceSubject.tpl',
        'School/Mail/ExtendedCare/InvoiceMessage.tpl',
        $value,
        null );
