...
class School_Utils_Mail {

  const
    School_FROM_EMAIL = "Parent Portal <info.portal@sfschool.org>",
    School_BCC_EMAIL  = "Parent Archival <archive.portal@sfschool.org>";

  static function sendMailToParents(
    $childID,
    $subjectTPL,
    $messageTPL,
    $templateVars,
    ...
    if ( $onlyEnrolled &&
      ! School_Utils_Query::isCurrentlyEnrolled( $childID ) ) {
      return;
    }

    require_once 'School/Utils/Relationship.php';
    $parentInfo = array( );
    School_Utils_Relationship::getParents( $childID, $parentInfo, false );
    ...
    list( $templateVars['childName'],
      $templateVars['childEmail'] ) = School_Utils_Query::getNameAndEmail( $childID );
