...
class School_Utils_Query {

  static function checkSubType( $id, $subType = 'Student', $redirect = true ) {
    $entitySubTypes = CRM_Contact_BAO_Contact::getContactSubType( $id );
    if ( ! is_array( $subType ) ) {
      $subType = array( $subType );
    }
    $flag = false;
    foreach ($subType as $entitySubType ) {
      if (  in_array( $entitySubType, $entitySubTypes ) ) {
        $flag =true;
        break;
      }
    }
...
";
    ...
    $params = array( 1 => array( School_Utils_ExtendedCare::getTerm( ), 'String' ) );
    $dao = CRM_Core_DAO::executeQuery( $sql, $params );
