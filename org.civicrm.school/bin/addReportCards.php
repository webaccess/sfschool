  ...
  foreach ( $grades as $grade ) {
    ...
    if ( ! empty( $gradeFiles ) ) {
      foreach ( $gradeFiles as $reportCard ) {
        ...
        if ( empty( $contactID ) ) {
          echo "$firstName, $lastName, $gradeDB\n";
...
";
  ...
  foreach ( $files as $idx =>& $file ) {
    echo "Validating {$file['firstName']}, {$file['lastName']}, {$file['gradeDB']}, {$file['path']}\n";
    ...
    // for Ubuntu
    $cmd = "/usr/bin/pdfgrep -ic \"$name\" {$file['path']} ";
    $count = exec( $cmd, $dontCare );
...
function run( $inputDir, $year, $term ) {
  ...
  $sql = generateSQL( $files, $year, $term, 1199, 1199, 1199 );

  print_r( $sql );
}

run(
  '/home/lobo/SFS/Reports',
  '2013-2014',
  'S2'
);
