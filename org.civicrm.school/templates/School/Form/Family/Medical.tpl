{crmDBTpl context='school family config' name='medical tab info' var='tabInfo'}
<div id="help">
{if !empty($tabInfo) }
 {$tabInfo}
{else}
...
{if empty($sections) || in_array(health, $sections) }
<br />
...
{if empty($sections) || in_array(counseling, $sections) }
<br />
</span>
<strong>School Counseling Permission</strong>
{crmDBTpl context='school family config' name='medical tab Counseling 1 text' var='tabInfo'}
{if !empty($tabInfo)}
{$tabInfo}
{else}
<p>
...
function confirmClicks( ) {
  if ( !cj('#counselor_authorization').attr('checked') ) {
